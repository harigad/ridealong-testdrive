...
class user {
    ...
    function addFeed($typeid,$ouid,$cid,$data=null,$created = null){
    	global $db;
    	
    	
    	$feed_data["typeid"] = $typeid;
    	$feed_data["uid"] = $this->id;
    	$feed_data["ouid"] = $ouid;
    	$feed_data["cid"] = $cid;
    	$feed_data["data"] = $data;
    	if($created){
    		$feed_data["created"] = $created;
    	}
    	$db->insert("feed",$feed_data);
    ...
    function init($fbid) {
        global $db;
        $row = $db->selectRow("select id,name,photo,photo_big,type from user where fbid='$fbid'");
	...
	function get_notices(){
		global $db;
		$recieved = $db->selectRows("select user.id, user.name,user.photo,user.photo_big ,car.cid,model.name as model ,make.name as make,checkin_request.id as noticeid,checkin_request.created from checkin_request 

        inner join owner on checkin_request.plate = owner.plate and  checkin_request.state = owner.state and owner.uid = {$this->id} 
